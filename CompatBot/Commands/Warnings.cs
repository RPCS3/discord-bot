...
internal static partial class Warnings
{
        ...
        AddAsync(ulong userId, DiscordUser issuer, string reason, string? fullReason = null, bool assignRole = false)
    {
        ...
        internal static async ValueTask Warn(SlashCommandContext ctx, DiscordMessage? message = null, DiscordUser? user = null)
    {
            ...
            && await guild.GetRoleAsync(Config.WarnRoleId).ConfigureAwait(false) is DiscordRole role)
        {
            if (await ctx.Client.GetMemberAsync(guild, user).ConfigureAwait(false) is DiscordMember member)
                modal.AddCheckbox(new("add_role"), $"Add {role.Name} role for member {member.DisplayName}".Trim(45));
            else
                modal.AddCheckbox(new("add_role"), $"Add {role.Name} role for user {user.DisplayName}".Trim(45));
        }
        ...
        try
        {
            ...
            var addRole = modalResult.Result.Values.TryGetValue("add_role", out var item)
                          && item is CheckboxModalSubmission { Value: true };
            var result = await AddAsync(user.Id, ctx.User, reason, message?.Content.Sanitize(), addRole).ConfigureAwait(false);
            if (result.IsFailure())
            {
            ...
            await ListUserWarningsAsync(ctx.Client, interaction, user.Id, user.Username.Sanitize()).ConfigureAwait(false);
