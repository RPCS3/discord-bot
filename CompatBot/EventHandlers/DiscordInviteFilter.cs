        ...
        public static async Task<bool> CheckMessageForInvitesAsync(DiscordClient client, DiscordMessage message)
        {
            ...
            if (hasInvalidResults)
            {
                try
                {
                    ...
                    await client.ReportAsync("🛃 An unapproved discord invite", message, "In invalid or expired invite", null, null, ReportSeverity.Low).ConfigureAwait(false);
                ...
                {
                    Config.Log.Warn(e);
                    await client.ReportAsync("🛃 An unapproved discord invite", message, "In invalid or expired invite", null, null, ReportSeverity.Medium).ConfigureAwait(false);
            ...
            foreach (var invite in invites)
            {
                if (!await InviteWhitelistProvider.IsWhitelistedAsync(invite).ConfigureAwait(false))
                {
                    ...
                    await client.ReportAsync("🛃 An unapproved discord invite", message, reportMsg, null, null, ReportSeverity.Low).ConfigureAwait(false);
